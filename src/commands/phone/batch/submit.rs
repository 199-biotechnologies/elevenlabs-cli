...
pub async fn run(
    ...
    if recipients_json.is_empty() {
        return Err(AppError::InvalidInput(
            "recipients list is empty — supply at least one phone number".into(),
        ));
    }
    ...
    let text = if source == "-" {
        ...
        if !path.exists() {
            return Err(AppError::InvalidInput(format!(
                "recipients file does not exist: {}",
                path.display()
            )));
        }
...
fn parse_recipients_json(text: &str) -> Result<Vec<serde_json::Value>, AppError> {
    let parsed: serde_json::Value = serde_json::from_str(text)
        .map_err(|e| AppError::InvalidInput(format!("recipients JSON is invalid: {e}")))?;
    let arr = parsed.as_array().ok_or_else(|| {
        AppError::InvalidInput("recipients JSON must be an array of objects".into())
    })?;
    let mut out = Vec::with_capacity(arr.len());
    for (idx, item) in arr.iter().enumerate() {
        let obj = item.as_object().ok_or_else(|| {
            AppError::InvalidInput(format!(
                "recipients JSON entry #{} must be an object",
                idx + 1
            ))
        })?;
        if !obj.contains_key("phone_number") {
            return Err(AppError::InvalidInput(format!(
                "recipients JSON entry #{} is missing phone_number",
                idx + 1
            )));
        }
...
fn parse_recipients_csv(text: &str) -> Result<Vec<serde_json::Value>, AppError> {
    ...
    for (idx, row) in iter.enumerate() {
        ...
        if phone.is_empty() {
            return Err(AppError::InvalidInput(format!(
                "recipients CSV row #{} has empty phone_number",
                idx + 1
            )));
        }
...
fn parse_csv_rows(text: &str) -> Result<Vec<Vec<String>>, AppError> {
    ...
    if in_quotes {
        return Err(AppError::InvalidInput(
            "recipients CSV has an unterminated quoted cell".into(),
        ));
    }
