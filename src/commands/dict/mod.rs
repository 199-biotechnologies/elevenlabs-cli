...
pub fn parse_rule(raw: &str) -> Result<serde_json::Value, AppError> {
    let (word, phoneme) = split_once_colon(raw).ok_or_else(|| {
        AppError::InvalidInput(format!(
            "--rule must be WORD:PHONEME (got '{raw}'). Example: --rule tomato:təˈmɑːtoʊ"
        ))
    })?;
    let word = word.trim();
    ...
    if word.is_empty() {
        return Err(AppError::InvalidInput(format!(
            "--rule missing WORD before ':' (got '{raw}')"
        )));
    }
    if phoneme.is_empty() {
        return Err(AppError::InvalidInput(format!(
            "--rule missing PHONEME after ':' (got '{raw}')"
        )));
    }
    Ok(serde_json::json!({
...
pub fn parse_alias_rule(raw: &str) -> Result<serde_json::Value, AppError> {
    let (word, alias) = split_once_colon(raw).ok_or_else(|| {
        AppError::InvalidInput(format!(
    ...
    if word.is_empty() {
        return Err(AppError::InvalidInput(format!(
            "--alias-rule missing WORD before ':' (got '{raw}')"
        )));
    }
    if alias.is_empty() {
        return Err(AppError::InvalidInput(format!(
            "--alias-rule missing ALIAS after ':' (got '{raw}')"
        )));
    }
    Ok(serde_json::json!({
        "string_to_replace": word,
...
pub fn collect_rules(
    ...
    if rules.is_empty() && alias_rules.is_empty() {
        return Err(AppError::InvalidInput(
            "at least one --rule WORD:PHONEME or --alias-rule WORD:ALIAS is required".into(),
        ));
    }
